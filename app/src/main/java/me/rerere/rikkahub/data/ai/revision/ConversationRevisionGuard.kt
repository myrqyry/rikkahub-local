...
sealed interface RevisionCheckResult {
    ...
    data class ConversationMissing(val conversationId: String) : RevisionCheckResult
...
class ConversationRevisionGuard(
    ...
    suspend fun check(
        ...
        val current = source.currentState(snapshot.conversationId) ?: return RevisionCheckResult.ConversationMissing(snapshot.conversationId)
        if (current.branchId != snapshot.branchId) {
            return RevisionCheckResult.BranchChanged(snapshot.branchId, current.branchId)
        }
        return when {
            current.revision == snapshot.revision -> RevisionCheckResult.Match
            policy == RevisionCommitPolicy.ALLOW_DESCENDANT_REVISION && current.revision > snapshot.revision -> RevisionCheckResult.Match
            current.revision < snapshot.revision -> RevisionCheckResult.RevisionRegressed(snapshot.revision, current.revision)
            else -> RevisionCheckResult.RevisionAdvanced(snapshot.revision, current.revision)
        }
    }
}
