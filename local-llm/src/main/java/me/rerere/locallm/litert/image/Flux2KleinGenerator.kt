...
class Flux2KleinGenerator(
  ...
  private val textToImageInputs = StagedInputs(packageRoot.binsDir)

  /** Editing inputs are only staged when the editing graphs are, so read them on demand. */
  private val editingInputs: StagedInputs by lazy { StagedInputs(File(packageRoot.root, EDITING_BINS)) }

  /** True when the editing graphs and their staged inputs are present on the device. */
  fun isEditingAvailable(): Boolean =
    packageRoot.graph("${EDITING_PREFIX}_final.tflite").exists() &&
      packageRoot.graph("kv_vae_enc.tflite").exists() &&
      File(File(packageRoot.root, EDITING_BINS), "patch_perm.bin").exists()

  /** True when the tokenizer and embedding table are staged, so the prompt can be typed. */
  fun isPromptEditable(): Boolean = Flux2KleinPromptEncoder.isStaged(packageRoot.tokenizerDir)
  ...
  fun generate(
    ...
    if (prompt != null && isPromptEditable()) {
      ...
      Flux2KleinPromptEncoder(packageRoot.tokenizerDir).use { encoder ->
    ...
    val pixels = Flux2KleinChunkRunner.gpu(environment, packageRoot.graph("kv_vae.tflite"), listOf(latentImage))[0]
  ...
  private fun encodeReference(reference: Bitmap, inputs: StagedInputs): FloatArray {
    ...
    val latent = Flux2KleinChunkRunner.gpu(environment, packageRoot.graph("kv_vae_enc.tflite"), planar)[0]
  ...
  private fun encodePrompt(
    ...
    for (index in 0 until ENCODER_CHUNKS) {
      hidden =
        Flux2KleinChunkRunner.gpu(
          environment,
          packageRoot.graph("ke_enc$index.tflite"),
  ...
  private fun denoiseStep(
    ...
    val prep =
      Flux2KleinChunkRunner.gpu(
        environment,
        packageRoot.graph("${prefix}_prep.tflite"),
    ...
    for (index in 0 until DOUBLE_CHUNKS) {
      val outputs =
        Flux2KleinChunkRunner.gpu(
          environment,
          packageRoot.graph("${prefix}_double$index.tflite"),
    ...
    for (index in 0 until SINGLE_CHUNKS) {
      joint =
        Flux2KleinChunkRunner.gpu(
          environment,
          packageRoot.graph("${prefix}_single$index.tflite"),
          listOf(joint, inputs.cos, inputs.sin, singleModulation),
        )[0]
    }
    return Flux2KleinChunkRunner.gpu(
      environment,
      packageRoot.graph("${prefix}_final.tflite"),
