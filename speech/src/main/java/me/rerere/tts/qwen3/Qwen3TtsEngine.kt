 ...
 * tokenization, embedding-table lookups, prompt assembly, and sampling.
 *
 * Kotlin port of the litert-samples Qwen3-TTS sample host loop
 * (Apache-2.0), which reproduces the PyTorch implementation token-for-token
 * under greedy decoding.
 *
 * All model files live in [dir].
 */
class Qwen3TtsEngine(private val dir: File) {
    ...
    /** Exposes plain-text tokenization (used by tests / self-test). */
    ...
    private fun load(name: String, threads: Int): CompiledModel {
        val f = File(dir, name)
        check(f.exists()) { "Model not found: $name" }
